...
class DealerList : ViewModel() {
    ...
    /*private fun fetchSecondApi(client: OkHttpClient, token: String?) {
        ...
        client.newCall(request2).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                val responseBody = response.body?.string()
                Log.d("SecondApi", "Second API Response Code: ${response.code}")
                Log.d("SecondApi", "Second API Response Body: $responseBody")

                if (response.isSuccessful) {
                    try {
                        responseBody?.let {
                            val jsonObject = JSONObject(it) // Extract root object
                            val carsArray = jsonObject.getJSONArray("cars") // Get "cars" array
                            ...
                            for (i in 0 until carsArray.length()) {
                                ...
                                val slot = Live(
                                    ...
                                    demand = carJson.getDouble("demand").toString(),
                                    photos = carPhotosList,
                                    overallRating = carJson.getString("overall_rating")
                                )
